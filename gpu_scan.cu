...
#include <iostream>
#include "hash.cuh"
#include "secp256k1_math.cuh" // Real privkey * G + pubkey compression

__global__ void scan_kernel(uint64_t start, uint64_t total,
                            const uint8_t* d_targets, size_t num_targets,
    ...
    // Construct 32-byte private key from 64-bit integer (right-aligned)
    uint8_t priv_bytes[32] = {0};
    for (int i = 0; i < 8; ++i)
        priv_bytes[24 + i] = (key >> ((7 - i) * 8)) & 0xFF;

    uint8_t pubkey[33], sha[32], h160[20];
    generate_compressed_pubkey(priv_bytes, pubkey); // Real ECC math
